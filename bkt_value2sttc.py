...
def value2sttc_long(data_score):
    ...
    _ = ((1-0.23/10000) * (Ilong*V1+Ishort*V0) - (1+0.23/10000) * Ilong * V0 -  (1+3.45/10000) * Ishort * V1) /(V0 * (1+0.23/10000))
    ...
    return_annual = (cashflow_temp['value_with_comm'][-1]/cashflow_temp['value_with_comm'][0]-1)  * 365 / (cash_value.index[-1]- cash_value.index[0]).days
    ...
    sttc_rslt = pd.DataFrame(data = [return_annual, return_ref ,std_annual, sharpe_annual, 
        ...
        (sttc_per_trade.loc[(sttc_per_trade['pos'] == 1),'return_per_trade'] > 0).sum() / (sttc_per_trade['pos'] == 1).sum() if (sttc_per_trade['pos'] == 1).sum() else np.inf,
        (sttc_per_trade['pos'] == -1).sum() /daycount,
        (sttc_per_trade.loc[(sttc_per_trade['pos'] == -1),'return_per_trade'] > 0).sum() / (sttc_per_trade['pos'] == -1).sum() if (sttc_per_trade['pos'] == -1).sum() else np.inf,
        ],dtype=float,index = ['收益率(年化)','收益率(Buy and Hold, 参考)','标准差(年化)','夏普比率(年化)','最大回撤水平','最大回撤比率','平均每日开仓次数','胜率',
            '平均每日开多仓次数','多仓胜率','平均每日开空仓次数','空仓胜率'])
    sttc_rslt = sttc_rslt.round(4)
    sttc_rslt.loc[(sttc_rslt.index.str.contains("率")) & (sttc_rslt[0] != np.inf)] = sttc_rslt.loc[(sttc_rslt.index.str.contains("率")) & (sttc_rslt[0] != np.inf)].applymap(lambda x: "{:.2%}".format(x))
